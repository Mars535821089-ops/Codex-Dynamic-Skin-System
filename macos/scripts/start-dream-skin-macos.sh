...
guard_against_submitted_keepalive_start || exit $?
OPERATION_TOKEN=""
...
[ "$REPAIR_WATCHER_ONLY" != "true" ] || {
  [ "$RESTART_EXISTING" = "false" ] && [ "$PROMPT_RESTART" = "false" ] \
    && [ "$FOREGROUND_INJECTOR" = "false" ] \
    || fail "Watcher-only repair cannot be combined with restart or foreground options."
}
...
if [ "$REPAIR_WATCHER_ONLY" = "true" ] && [ "$DEBUG_READY" != "true" ]; then
  fail "Watcher-only repair requires the existing verified Codex CDP endpoint; refusing to restart Codex."
fi
...
if [ "$DEBUG_READY" = "false" ]; then
  ...
  if [ "$FOREGROUND_INJECTOR" != "true" ]; then
    ...
    if ! recorded_injector_process_matches "$INJECTOR_PID" "$INJECTOR_STARTED_AT" \
      "$NODE" "$INJECTOR" "$PORT"; then
      remove_injector_launchd_job
      fail "Could not verify the launched injector process identity. See $INJECTOR_ERROR_LOG"
    fi
  fi
  if ! wait_for_cdp "$PORT"; then
    remove_injector_launchd_job
    [ -z "$INJECTOR_PID" ] || signal_recorded_injector_process TERM "$INJECTOR_PID" \
      "$INJECTOR_STARTED_AT" "$NODE" "$INJECTOR" "$PORT" || true
...
if [ "$REPAIR_WATCHER_ONLY" != "true" ]; then
  activate_codex_window
fi

if [ "$FOREGROUND_INJECTOR" = "true" ]; then
  BACKGROUND_PLAYBACK_FLAG=""
  if codex_background_playback_capable; then
    BACKGROUND_PLAYBACK_FLAG="--background-playback-capable"
  fi
  exec "$NODE" "$INJECTOR" --watch --port "$PORT" --theme-dir "$THEME_DIR" \
    ...
    ${BACKGROUND_PLAYBACK_FLAG:+"$BACKGROUND_PLAYBACK_FLAG"} \
...
if [ -z "$INJECTOR_STARTED_AT" ]; then
  INJECTOR_STARTED_AT="$(process_started_at "$INJECTOR_PID")"
fi
[ -n "$INJECTOR_STARTED_AT" ] || fail "Could not record the injector process start time."
recorded_injector_process_matches "$INJECTOR_PID" "$INJECTOR_STARTED_AT" \
  "$NODE" "$INJECTOR" "$PORT" || fail "The injector process identity changed during startup."
...
if run_injector_verify "$PORT" "$CURRENT_THEME_DIR" 20000 >"$VERIFY_OUTPUT" 2>/dev/null; then
  verify_code=0
else
  verify_code=$?
fi
if [ "$verify_code" -ne 0 ]; then
  # A slow App startup may register its reopen handler after CDP. Keep the
  # watcher as the sole renderer owner, reactivate the exact bundle, and give
  # its discovery loop one final verification window.
  [ "$REPAIR_WATCHER_ONLY" = "true" ] || activate_codex_window
  CURRENT_THEME_DIR="$(resolve_current_theme_dir)" || fail "Could not resolve the currently selected theme."
  if run_injector_verify "$PORT" "$CURRENT_THEME_DIR" 12000 >"$VERIFY_OUTPUT" 2>/dev/null; then
    verify_code=0
  else
    verify_code=$?
  fi
