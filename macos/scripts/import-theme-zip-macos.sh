...
if [ -n "$EXPECTED_SHA256" ]; then
  ...
  [ "$EXPECTED_BYTES" -gt 0 ] || fail "Expected package byte count is outside the import limit."
fi
...
DREAMSKIN_NODE="$NODE" "$SCRIPT_DIR/extract-theme-zip-macos.sh" "$ARCHIVE_SNAPSHOT" "$EXTRACT_STAGE"
...
# Package metadata has already been authenticated by the shared validator and
# is consumed by the publisher. It is not part of the installed renderer asset
# set, so exclude it only from this disposable runtime-payload probe.
/bin/rm -f "$PAYLOAD_VALIDATION_STAGE/manifest.json" "$PAYLOAD_VALIDATION_STAGE/manifest.sig"
"$NODE" -e '
