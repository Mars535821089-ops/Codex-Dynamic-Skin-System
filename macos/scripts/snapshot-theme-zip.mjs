...
const contract = JSON.parse(await fs.readFile(
  new URL("../assets/dynamic/theme-contract.json", import.meta.url),
  "utf8",
));
const MAX_ARCHIVE_BYTES = contract?.["x-limits"]?.zipCompressedBytes;
if (!Number.isSafeInteger(MAX_ARCHIVE_BYTES) || MAX_ARCHIVE_BYTES < 1) {
  throw new Error("Generated theme contract has an invalid ZIP archive limit");
}
...
async function main() {
  ...
  try {
    ...
    if (before.size > BigInt(MAX_ARCHIVE_BYTES)) {
      throw new Error(`Theme ZIP exceeds the ${MAX_ARCHIVE_BYTES}-byte archive limit`);
    }
    ...
    for (;;) {
      ...
      if (copied > MAX_ARCHIVE_BYTES) {
        throw new Error(`Theme ZIP exceeds the ${MAX_ARCHIVE_BYTES}-byte archive limit while being copied`);
      }
      let written = 0;
