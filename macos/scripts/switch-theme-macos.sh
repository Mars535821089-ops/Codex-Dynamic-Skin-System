...
THEME_SCHEMA_VERSION="$("$NODE" -e '
const value = JSON.parse(process.argv[1]);
if (![1, 2].includes(value.schemaVersion)) process.exit(1);
process.stdout.write(String(value.schemaVersion));
' "$STAGE_RESULT")" || fail "Theme staging returned an invalid schema identity."
THEME_IMAGE=""
if [ "$THEME_SCHEMA_VERSION" = "1" ]; then
  THEME_IMAGE="$("$NODE" -e '
  const value = JSON.parse(process.argv[1]);
...
# Synchronize the complete validated v1/v2 runtime tree while preserving the
# old theme.json as the commit marker. A failure copying nested media or
# removing stale files aborts before the active config can change.
/usr/bin/rsync -a --delete --exclude='/theme.json' "$stage/" "$THEME_DIR/"
# theme.json is the final commit marker: the watcher never observes metadata
# that references a partially copied nested media tree.
/bin/mv -f "$stage/theme.json" "$THEME_DIR/theme.json"
/bin/rm -rf "$stage"
stage=""

# Keep the watcher restart source of truth aligned with switches made from the
# native app/menu path. The theme center writes this same atomic marker when it
# switches directly through CDP.
ACTIVE_THEME_ID="$("$NODE" -e 'const t=JSON.parse(require("fs").readFileSync(process.argv[1],"utf8"));if(!/^[A-Za-z0-9][A-Za-z0-9._-]{0,79}$/.test(t.id||""))process.exit(1);process.stdout.write(t.id)' "$THEME_DIR/theme.json")" \
  || fail "Published theme has an invalid id."
"$NODE" "$SCRIPT_DIR/theme-selection-store.mjs" --write \
  "$STATE_ROOT/selected-theme.json" "$ACTIVE_THEME_ID" \
  || fail "Could not persist the selected theme."
...
if hot_reapply_theme "$PORT" 8000 "$OPERATION_TOKEN"; then
  ...
  run_injector_verify "$PORT" "$THEME_DIR" 10000 >/dev/null \
