...
export async function finalizeThemeLibraryMigration(result) {
  // A completed copy is not sufficient: an external disk can disappear or the
  ...
  for (const archive of result?.archives ?? []) {
    if (await treeDigest(archive.destination) !== archive.digest) {
      throw new Error(`Archived destination changed before migration cleanup: ${path.basename(archive.destination)}`);
    }
  }
  for (const theme of result?.themes ?? []) {
  ...
  for (const theme of result?.themes ?? []) await fs.rm(theme.source, { recursive: true, force: true });
  for (const archive of result?.archives ?? []) await fs.rm(archive.source, { recursive: true, force: true });
...
export async function migrateThemeLibrary({ sourceRoot, destinationRoot, removeSource = true }) {
  ...
  if (deletedDestinationExisting && path.dirname(deletedDestinationExisting) !== destination) {
    throw new Error("Theme archive escaped its destination library");
  }
  const directories = [];
  ...
  try {
    ...
    const deletedEntries = deletedSource ? await fs.readdir(deletedSource, { withFileTypes: true }) : [];
    if (deletedEntries.length) {
      const deletedDestination = path.join(destination, ".deleted");
      await fs.mkdir(deletedDestination, { recursive: true, mode: 0o700 });
      const trustedDeletedDestination = await existingDirectory(deletedDestination, "Theme archive");
      if (path.dirname(trustedDeletedDestination) !== destination) throw new Error("Theme archive escaped its destination library");
