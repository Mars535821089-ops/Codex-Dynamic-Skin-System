...
let limits;
try {
  const contractPath = path.join(scriptRoot, "..", "assets", "dynamic", "theme-contract.json");
  const source = JSON.parse(fs.readFileSync(contractPath, "utf8"))?.["x-limits"];
...
function decodeName(buffer) {
  try {
    ...
    fail("ASSET_PATH", "entry name is not valid UTF-8");
  }
}
...
console.log(JSON.stringify({ entries: totalEntries, expandedBytes: expanded, limits }));
