...
await test("macOS ZIP snapshot and extraction boundaries", {
  ...
  try {
    await oversizedHandle.truncate((128 * 1024 * 1024) + 1);
  } finally {
    await oversizedHandle.close();
  }
  await expectSnapshotRejected(oversizedArchive, oversizedSnapshot, /134217728-byte archive limit/);
  ...
  await fs.writeFile(forgedArchive, forgedExpandedZip(97 * 1024 * 1024, 1));
  assert.ok((await fs.stat(forgedArchive)).size < 1024 * 1024);
  await fs.mkdir(forgedDestination);
  await assert.rejects(run(extractor, [forgedArchive, forgedDestination]), /ZIP_INVALID|ZIP_LIMIT/);
