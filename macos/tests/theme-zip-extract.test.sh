...
expect_structurally_accepted() {
  local archive="$1"
  local label="$2"
  local destination="$TMP/accepted-$label"
  /bin/mkdir -p "$destination"
  "$EXTRACTOR" "$archive" "$destination"
  [ -f "$destination/theme.json" ] \
    || { printf 'Structurally valid ZIP did not stage theme.json for %s.\n' "$label" >&2; exit 1; }
}
...
expect_structurally_accepted "$TMP/official-without-css.zip" official-without-css

(
  cd "$TMP/root-pack"
  /usr/bin/zip -q "$TMP/simple-without-css.zip" theme.json background.jpg
)
expect_structurally_accepted "$TMP/simple-without-css.zip" simple-without-css
...
/usr/sbin/mkfile 97m "$TMP/large-pack/background.jpg"
(
  cd "$TMP/large-pack"
...
for index in $(/usr/bin/jot 65 1); do
...
expect_structurally_accepted "$TMP/unknown.zip" unregistered-official-file

printf 'PASS: macOS ZIP extraction separates structure safety from package semantics.\n'
