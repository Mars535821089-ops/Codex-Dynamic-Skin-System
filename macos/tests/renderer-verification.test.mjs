...
function makeDomFixture({
  ...
  const document = {
    ...
    querySelectorAll: (selector) => selector === "[data-dynamic-skin-root]"
      ? dynamicRoots : [],
...
test("Skin API v2 requires exactly one visible dynamic root", async () => {
  ...
  const cases = [
    ["missing root", []],
    ["duplicate roots", [makeElement(), makeElement()]],
    ["CSS-hidden root", [makeElement({ style: { display: "none" } })]],
  ];

  const healthy = await verifySession(
    makeSession({ dom: makeDomFixture({ dynamic: activeDynamic, dynamicRoots: [makeElement()] }) }),
    "fixture-theme",
    "fixture-revision",
    true,
  );
  assert.equal(healthy.pass, true, "one visible root must satisfy the API v2 ownership gate");
  assert.equal(healthy.dynamicRoots.length, 1);
  assert.equal(healthy.dynamicRoots[0].visible, true);

  for (const [label, dynamicRoots] of cases) {
    const result = await verifySession(
      makeSession({ dom: makeDomFixture({ dynamic: activeDynamic, dynamicRoots }) }),
      "fixture-theme",
      "fixture-revision",
      true,
    );
    assert.equal(result.pass, false, label);
    assert.equal(result.checks.dynamicRootPass, false, label);
  }
});
