...
const dynamicPayload = { ...exactPayload, expectedDynamic: true };
assert.equal(
  assessRendererVerification(baseRenderer, readyNativeWindow, dynamicPayload).pass,
  false,
  "Skin API v2 verification must not accept the legacy shell without an active dynamic controller.",
);
assert.equal(assessRendererVerification({
  ...baseRenderer,
  dynamic: { activation: "active", diagnostics: { phase: "active" } },
  dynamicRoots: [{ visible: true }],
}, readyNativeWindow, dynamicPayload).pass, true);
assert.equal(assessRendererVerification({
  ...baseRenderer,
  dynamicRoots: [{ visible: true }],
  dynamic: {
    ...
    diagnostics: {
      phase: "active",
      modules: {
        "media-layer": { mode: "video", playing: false, loop: true, backgroundPlayback: false },
      },
    },
  },
}, readyNativeWindow, dynamicPayload).pass, true,
"A deliberately paused video must verify when background playback is disabled.");
assert.equal(assessRendererVerification({
  ...baseRenderer,
  dynamicRoots: [{ visible: true }],
  dynamic: {
    ...
    diagnostics: {
      phase: "active",
      modules: {
        "media-layer": { mode: "video", playing: false, loop: true, backgroundPlayback: true },
      },
    },
  },
}, readyNativeWindow, dynamicPayload).pass, false,
"A paused video must still fail verification when background playback is enabled.");
...
assert.match(
  startSource,
  /if \[ "\$FOREGROUND_INJECTOR" = "true" \]; then[\s\S]*?codex_background_playback_capable[\s\S]*?BACKGROUND_PLAYBACK_FLAG="--background-playback-capable"[\s\S]*?\$\{BACKGROUND_PLAYBACK_FLAG:\+"\$BACKGROUND_PLAYBACK_FLAG"\}/,
  "The foreground watcher must derive background-playback capability from the live Codex process.",
);
...
dreamskin_text() { /usr/bin/printf '%s\\n' "$1"; }
guard_against_submitted_keepalive_start() { return 0; }
...
run_injector_verify() {
  port="$1"; theme_dir="$2"; timeout_ms="$3"; shift 3
  "$NODE" "$INJECTOR" --verify --port "$port" --theme-dir "$theme_dir" \
    --theme-library "$STATE_ROOT/themes" --settings "$STATE_ROOT/dynamic-settings.json" \
    --background-playback-capable --timeout-ms "$timeout_ms" "$@"
}
...
stop_codex() { return 0; }
resolve_current_theme_dir() { /usr/bin/printf '%s\n' "$THEME_DIR"; }
...
`, { mode: 0o700 });
  ...
  await assert.rejects(
    fs.stat(environment.NODE_LOG),
    (error) => error?.code === "ENOENT",
    "Startup recovery must not invoke a second renderer writer.",
  );
  ...
  await assert.rejects(
    fs.stat(environment.NODE_LOG),
    (error) => error?.code === "ENOENT",
    "Failed startup recovery must not invoke a second renderer writer.",
  );
