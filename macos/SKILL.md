...
- The default background monitor may repair only a watcher attached to an already compliant Codex process. It must not quit, signal, restart, or activate an ordinary Codex process. `--allow-automatic-codex-restart` is an elevated opt-in whose `TERM`/`KILL` escalation must be disclosed.
- Restore must leave a persistent native-mode intent, including across a failed reinstall and its rollback. Only a successful reinstall or explicit monitor `enable` may clear it. Uninstall must preflight first and resume a previously running monitor if a later step fails.
