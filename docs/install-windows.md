...
For automatic injection when using the ordinary Codex icon, enable **Auto-inject on normal Codex launch (idle restart only)** in the tray. This is separate from **Launch at login**; enable both to retain the behavior after a computer restart. The normal launch gets a 10-second grace period, followed by a check of local task lifecycle records. Only confirmed idle state allows one graceful restart with the required debugging flags. Running tasks, missing or ambiguous evidence, an isolated profile, and ambiguous process identity prevent automatic restart. Attempts have a five-minute cooldown. The worker sets a durable restart latch under the operation lock immediately before requesting the graceful close, after its safety checks; a refusal before that point does not permanently prevent later injection. Even after the observer or computer restarts, clearing the latch requires ten seconds of continuously confirmed closure, the operation lock and a fresh empty process inventory. Unknown state never clears it. The observer never runs an AI CLI or submits a chat message.
