...
- Runtime trust regressions execute the real validation functions with signature and process boundaries mocked: invalid signatures and unexpected publishers must be rejected before any Node execution; malformed version and executable-identity responses must fail closed. The ZIP fixture resolves one real signed runtime per suite, then reuses only that verified test runtime while all media and package validators still execute normally. Production runtime validation is unchanged.
