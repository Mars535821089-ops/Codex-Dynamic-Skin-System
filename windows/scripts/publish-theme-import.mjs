...
import { THEME_LIMITS, validateThemeDefinition } from "../assets/dynamic/theme-contract.mjs";
import {
...
const MAX_RUNTIME_ASSET_BYTES = THEME_LIMITS.singleEntryBytes;
const MAX_RUNTIME_FILES = THEME_LIMITS.zipEntries;
const MAX_THEME_ID_LENGTH = 128;
const LEGACY_THEME_ID_LENGTH = 80;
...
function decodeTheme(bytes, label) {
  ...
  if (!theme || typeof theme !== "object" || Array.isArray(theme)
      || ![1, 2].includes(theme.schemaVersion)) {
    throw new Error(`${label} must use theme schemaVersion 1 or 2`);
  }
  if (theme.schemaVersion === 1
      && (typeof theme.image !== "string" || !theme.image || path.basename(theme.image) !== theme.image)) {
...
function treeFingerprint(theme, files, { includeId }) {
  return runtimeThemeTreeFingerprint(theme, files, { includeId });
}

async function collectRuntimeTree(directory) {
  const files = new Map();
  async function walk(current, prefix = "") {
    for (const entry of await fs.readdir(current, { withFileTypes: true })) {
      const relative = prefix ? `${prefix}/${entry.name}` : entry.name;
      if (["manifest.json", "manifest.sig", "LICENSE.txt"].includes(relative)) continue;
      if (entry.isSymbolicLink()) throw new Error(`Runtime theme contains a symbolic link: ${relative}`);
      const target = path.join(current, entry.name);
      if (entry.isDirectory()) await walk(target, relative);
      else if (entry.isFile()) {
        if (files.size >= MAX_RUNTIME_FILES) throw new Error("Runtime theme contains too many files");
        files.set(relative, await readRegular(target, `Runtime theme file ${relative}`, MAX_RUNTIME_ASSET_BYTES));
      } else {
        throw new Error(`Runtime theme contains an unsupported entry: ${relative}`);
      }
    }
  }
...
function safeBaseId(value, fingerprint, schemaVersion = 1) {
  ...
  const fallbackPrefix = schemaVersion === 2 ? "import." : "import-";
  if (!candidate) return `${fallbackPrefix}${fingerprint.slice(0, 24)}`;
  const identity = createHash("sha256")
    .update("dreamskin-source-theme-id/1\0")
    .update(candidate)
    .digest("hex");
  return `${fallbackPrefix}${identity.slice(0, 24)}`;
}
...
async function readStoredTheme(directory) {
  try {
    ...
    if (theme.schemaVersion === 2) {
      await loadInstalledSkin(directory, { platform: "windows", clientVersion: "1.5.17" });
      const files = await collectRuntimeTree(directory);
      return {
        theme,
        fingerprint: treeFingerprint(theme, files, { includeId: false }),
        contentFingerprint: treeFingerprint(theme, files, { includeId: true }),
        hasOnlyRuntimeFiles: true,
      };
    }
...
function legacySuffixOf(value, baseId) {
  ...
  const marker = `-${suffix}`;
  const candidatePrefix = value.slice(0, -marker.length);
  const matchesKnownFormat = [MAX_THEME_ID_LENGTH, LEGACY_THEME_ID_LENGTH].some(
    (limit) => candidatePrefix === baseId.slice(0, Math.max(0, limit - marker.length)),
  );
  if (!matchesKnownFormat) return null;
...
async function main() {
  ...
  if (sourceTheme.schemaVersion === 1) {
    const imagePath = path.join(stageRoot, sourceTheme.image);
    assertContained(stageRoot, imagePath, "Imported theme image");
    imageBytes = await readRegular(imagePath, "Imported theme image", MAX_IMAGE_BYTES);
    fingerprint = normalizedFingerprint(sourceTheme, imageBytes, cssBytes, licenseBytes);
    fallbackFingerprint = sourceIdFallbackFingerprint(sourceTheme, imageBytes, cssBytes, licenseBytes);
  } else {
    runtimeFiles = await collectRuntimeTree(stageRoot);
    validateThemeDefinition(
      sourceTheme,
      [...runtimeFiles.keys()].filter((fileName) => fileName !== "theme.json"),
    );
    fingerprint = treeFingerprint(sourceTheme, runtimeFiles, { includeId: false });
    fallbackFingerprint = fingerprint;
  }
  if (sourceTheme.schemaVersion === 1 && !cssBytes) throw new Error("New theme imports require non-empty theme.css");
  if (cssBytes) decodeAndValidateSafeCss(cssBytes);
  const safeCssStatus = cssBytes ? "validated" : "none";
  const releaseLock = await acquireLock(themesRoot);
  let temporary = "";
  try {
    ...
    if (!replaceExisting) {
      ...
      while (await pathExists(path.join(themesRoot, id))) {
        const marker = `-${suffix}`;
        id = `${baseId.slice(0, MAX_THEME_ID_LENGTH - marker.length)}${marker}`;
        suffix += 1;
      }
    }
    ...
    const contentFingerprint = sourceTheme.schemaVersion === 1
      ? runtimeThemeContentFingerprint(theme, imageBytes, cssBytes)
      : treeFingerprint(theme, runtimeFiles, { includeId: true });
    ...
    if (sourceTheme.schemaVersion === 1) {
      ...
      for (const [fileName, bytes] of runtimeFiles) {
        ...
        const target = path.join(temporary, ...fileName.split("/"));
        assertContained(temporary, target, `Imported runtime file ${fileName}`);
