...
$script:DreamSkinMaxThemeArchiveBytes = 128 * 1024 * 1024
$script:DreamSkinMaxThemeArchiveExpandedBytes = 256 * 1024 * 1024
$script:DreamSkinMaxThemeArchiveEntries = 64
$script:DreamSkinMaxThemeArchiveSingleEntryBytes = 96 * 1024 * 1024
...
function Expand-DreamSkinThemeZipSecurely {
  ...
  if ($archiveLength -gt $script:DreamSkinMaxThemeArchiveBytes) {
    throw 'Theme ZIP exceeds the 128 MiB archive limit.'
  }
  ...
  if (-not (Get-Command Get-DreamSkinNodeRuntime -ErrorAction SilentlyContinue)) {
    throw 'Node.js runtime validation is unavailable for ZIP structure checks.'
  }
  $node = Get-DreamSkinNodeRuntime
  $engineRoot = Split-Path -Parent $PSScriptRoot
  $preflightPath = Join-Path $engineRoot 'assets\dynamic\zip-preflight.mjs'
  if (-not (Test-Path -LiteralPath $preflightPath -PathType Leaf)) {
    throw 'Theme ZIP structure preflight is missing from the runtime engine.'
  }
  $preflightOutput = @(& $node.Path $preflightPath $archiveFullPath 2>&1)
  if ($LASTEXITCODE -ne 0) {
    $detail = ($preflightOutput -join "`n").Trim()
    throw "Theme ZIP structure preflight failed: $detail"
  }
  try { $preflight = ($preflightOutput -join "`n") | ConvertFrom-Json -ErrorAction Stop } catch {
    throw 'Theme ZIP structure preflight returned invalid output.'
  }
  if ($preflight.archiveSha256 -isnot [string] -or
    $preflight.archiveSha256 -cnotmatch '\A[a-f0-9]{64}\z' -or
    [int64]$preflight.entries -lt 1 -or
    [int64]$preflight.entries -gt $script:DreamSkinMaxThemeArchiveEntries -or
    [int64]$preflight.expandedBytes -lt 0 -or
    [int64]$preflight.expandedBytes -gt $script:DreamSkinMaxThemeArchiveExpandedBytes) {
    throw 'Theme ZIP structure preflight returned an invalid summary.'
  }
  ...
  try {
    ...
    if ($hasExpectedBytes -and $openedArchiveLength -ne $ExpectedArchiveBytes) {
    ...
    if ($openedArchiveSha256 -cne "$($preflight.archiveSha256)" -or
      ($hasExpectedSha -and $openedArchiveSha256 -cne $ExpectedArchiveSha256)) {
      throw 'Theme ZIP SHA-256 changed after preflight or does not match the approved package identity.'
    }
    ...
    if ($entries.Count -gt $script:DreamSkinMaxThemeArchiveEntries) {
      throw 'Theme ZIP exceeds the 64-entry limit.'
    }
    ...
    foreach ($entry in $entries) {
      ...
      $normalized = $rawName.Normalize([System.Text.NormalizationForm]::FormC)
      if ($normalized -cne $rawName -or $normalized.Contains('\') -or
      ...
      if ($entryLength -gt $script:DreamSkinMaxThemeArchiveSingleEntryBytes) {
        throw 'Theme ZIP entry exceeds the 96 MiB single-entry limit.'
      }
      if ($expandedBytes -gt ([int64]::MaxValue - $entryLength)) {
        throw 'Theme ZIP expanded-size counter overflowed.'
      }
      $expandedBytes = [int64]($expandedBytes + $entryLength)
      if ($expandedBytes -gt $script:DreamSkinMaxThemeArchiveExpandedBytes) {
        throw 'Theme ZIP exceeds the 256 MiB expanded-size limit.'
      }
      ...
      $relativePlatformPath = $trimmed.Replace('/', [System.IO.Path]::DirectorySeparatorChar)
      $destination = [System.IO.Path]::GetFullPath((Join-Path $destinationFullPath $relativePlatformPath))
      ...
      try {
        ...
        while (($read = $input.Read($buffer, 0, $buffer.Length)) -gt 0) {
          $written += $read
          if ($written -gt $entryLength -or
            $written -gt $script:DreamSkinMaxThemeArchiveSingleEntryBytes) {
...
function Invoke-DreamSkinThemeReplacementRecovery {
  ...
  try {
    ...
    $publisher = Join-Path $PSScriptRoot 'publish-theme-import.mjs'
    if (-not (Test-Path -LiteralPath $publisher -PathType Leaf)) {
      throw 'Dynamic theme recovery publisher is missing from the runtime engine.'
    }
    $node = Get-DreamSkinNodeRuntime
    $publisherOutput = @(& $node.Path $publisher '--recover' $Paths.Saved 2>&1)
    if ($LASTEXITCODE -ne 0) {
      $detail = ($publisherOutput -join "`n").Trim()
      throw $(if ($detail) { $detail } else { 'Dynamic theme replacement recovery failed.' })
    }
    try {
      $publishedRecovery = ($publisherOutput -join "`n") | ConvertFrom-Json -ErrorAction Stop
    } catch {
      throw 'Dynamic theme recovery publisher returned invalid output.'
    }
    if ($publishedRecovery.status -cne 'recovered') {
      throw 'Dynamic theme recovery publisher returned an invalid status.'
    }
  } finally {
...
function Import-DreamSkinThemeZip {
  ...
  try {
    ...
    $skinApiVersion = if ($null -ne $packageInfo.skinApiVersion) {
      [int]$packageInfo.skinApiVersion
    } else {
      1
    }
    if ($skinApiVersion -eq 2) {
      if ($safeCssStatus -notin @('validated', 'none')) {
        throw 'Dynamic theme ZIP returned an invalid Safe CSS status.'
      }
      $publisher = Join-Path $PSScriptRoot 'publish-theme-import.mjs'
      if (-not (Test-Path -LiteralPath $publisher -PathType Leaf)) {
        throw 'Dynamic theme publisher is missing from the runtime engine.'
      }
      $publisherOutput = @(& $node.Path $publisher $validatedRoot $paths.Saved 2>&1)
      if ($LASTEXITCODE -ne 0) {
        $detail = ($publisherOutput -join "`n").Trim()
        throw $(if ($detail) { $detail } else { 'Dynamic theme publication failed.' })
      }
      try {
        $published = ($publisherOutput -join "`n") | ConvertFrom-Json -ErrorAction Stop
      } catch {
        throw 'Dynamic theme publisher returned invalid output.'
      }
      if ($published.status -notin @('imported', 'duplicate') -or
        $published.id -isnot [string] -or -not $published.id) {
        throw 'Dynamic theme publisher returned an invalid result.'
      }
      return [pscustomobject]@{
        Status = if ($published.status -ceq 'imported') { 'Imported' } else { 'Duplicate' }
        Id = "$($published.id)"
        Name = "$($published.name)"
        Renamed = [bool]$published.renamed
        Replaced = [bool]$published.replaced
        NameCollision = [bool]$published.nameCollision
        PackageFormat = "$($published.packageFormat)"
        SafeCssStatus = "$($published.safeCssStatus)"
        SignatureIgnored = [bool]$published.signatureIgnored
        ContentFingerprint = "$($published.contentFingerprint)"
        CleanupWarning = if ($published.cleanupWarning) { "$($published.cleanupWarning)" } else { $null }
        Path = Join-Path $paths.Saved "$($published.id)"
      }
    }
