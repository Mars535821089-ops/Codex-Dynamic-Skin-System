...
已正常注入时不会反复重注入。普通启动先留 10 秒启动时间；有任务运行、无法确认空闲或实例身份不明确时，不自动重启。只有真正准备请求关闭 Codex 时才设置防重复重启标记；此前被安全检查拒绝的请求仅进入 5 分钟冷却，不会永久挡住后续注入。连续确认 Codex 已关闭 10 秒且没有启动操作正在进行后，才清除标记。暂停会保留，Codex 关闭时不会被自动拉起。视频后台播放默认关闭，降低资源消耗。
