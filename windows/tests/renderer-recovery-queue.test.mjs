...
test("coalesces repeated lifecycle events into one Windows renderer recovery", async () => {
  const reasons = [];
...
test("an obsolete Windows renderer record cannot mutate its replacement", async () => {
  let current = true;
  let recoveries = 0;
  const queue = createRendererRecoveryQueue({
    isCurrent: () => current,
...
test("a Windows renderer recovery failure is isolated and reported once", async () => {
  const failures = [];
  const queue = createRendererRecoveryQueue({
    isCurrent: () => true,
