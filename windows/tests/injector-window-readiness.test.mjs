...
function makeDomFixture({
  ...
  const document = {
    ...
    querySelectorAll(selector) {
      if (selector === '[data-dynamic-skin-root]') return dynamicRoots;
      return [];
    },
...
test("Skin API v2 requires exactly one visible dynamic root", async () => {
  const activeDynamic = {
    generation: "fixture-revision",
    activation: "active",
    diagnostics: () => ({ phase: "active", modules: {} }),
  };
  const dynamicFixture = (dynamicRoots) => makeDomFixture({ dynamic: activeDynamic, dynamicRoots });

  const missing = await verify({ dom: dynamicFixture([]) }, true);
  assert.equal(missing.result.pass, false,
    "Active diagnostics must not hide a missing v2 dynamic root.");
  assert.equal(missing.result.dynamicRootCount, 0);
  assert.equal(missing.result.visibleDynamicRootCount, 0);

  const visibleRoot = makeElement({ rect: makeRect(1280, 800) });
  const healthy = await verify({ dom: dynamicFixture([visibleRoot]) }, true);
  assert.equal(healthy.result.pass, true);
  assert.equal(healthy.result.dynamicRootCount, 1);
  assert.equal(healthy.result.visibleDynamicRootCount, 1);

  const duplicate = await verify({
    dom: dynamicFixture([visibleRoot, makeElement({ rect: makeRect(1280, 800) })]),
  }, true);
  assert.equal(duplicate.result.pass, false,
    "A duplicate v2 root must fail verification instead of allowing double rendering.");
  assert.equal(duplicate.result.dynamicRootCount, 2);
  assert.equal(duplicate.result.visibleDynamicRootCount, 2);

  const hidden = await verify({
    dom: dynamicFixture([makeElement({
      rect: makeRect(1280, 800),
      style: { visibility: "hidden" },
      visible: false,
    })]),
  }, true);
  assert.equal(hidden.result.pass, false,
    "A CSS-hidden v2 root must not count as an active theme.");
  assert.equal(hidden.result.dynamicRootCount, 1);
  assert.equal(hidden.result.visibleDynamicRootCount, 0);
});
...
test("watcher startup verification never launches a competing one-shot writer", async () => {
  const source = await fs.readFile(startPath, "utf8");
  const verifyStart = source.indexOf("$verifyDeadline =");
  const verifyEnd = source.indexOf("if ($null -ne $appearanceTransaction)", verifyStart);
  const verifyLoop = source.slice(verifyStart, verifyEnd);
  assert.doesNotMatch(verifyLoop, /--once/,
    "A running watcher is the sole renderer writer even when startup verification is slow.");
  assert.doesNotMatch(verifyLoop, /forceInjectedAfterVerifyFailure/);
});
