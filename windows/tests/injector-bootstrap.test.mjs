...
assert.match(source, /if \(earlyInjectionFallback\) attachLoadFallback\(/,
  "Load-event recovery must be attached only when early injection falls back.");
assert.match(source, /if \(paused \|\| !fallbackTargets\.get\(id\)\) return;/,
  "Fallback listeners must stay completely inert after a successful early registration.");
assert.match(source, /queue\?\.request\("Page\.loadEventFired", \{ delayMs: 250 \}\)/,
