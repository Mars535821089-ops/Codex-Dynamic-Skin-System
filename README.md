...
- 自动修复完全由本地程序执行，不调用模型或供应商 API，也不消耗 Token；若选择允许自动重启，会先给新启动的 Codex 10 秒宽限期，只有确认当前没有运行任务时才会重启，同一故障周期最多重启一次，活动状态不明时一律不重启
- 出错时保留原生界面，可验证、可恢复

为了避免 Codex 在后台持续占用 GPU/CPU，视频的“后台播放”默认关闭。只有明确打开该选项后，下次重启 Codex 才会启用完整的后台播放能力；开启后会增加资源占用。

如果主题库放在外置硬盘上，启动前请先确认硬盘已挂载。暂时无法读取主题库时，程序可使用本机备用主题，但不会因此覆盖已保存的主题选择；重新挂载后，下次从主题启动入口打开时会重新读取原选择。主动换选其他主题仍会正常保存。
