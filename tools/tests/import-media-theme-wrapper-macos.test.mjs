...
test("media import wrapper requires the signed bundled Node runtime", async (t) => {
  if (process.platform !== "darwin") return t.skip("macOS runtime discovery is required");
  if (process.env.CODEX_DREAM_SKIN_SKIP_SIGNED_RUNTIME_TESTS === "1") {
    return t.skip("CI runner does not provide the signed Codex application runtime");
  }
