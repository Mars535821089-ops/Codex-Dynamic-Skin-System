...
const signedRuntimeUnavailable = process.platform !== "darwin"
  || process.env.CODEX_DREAM_SKIN_SKIP_SIGNED_RUNTIME_TESTS === "1";

if (signedRuntimeUnavailable) {
  test("signed Codex runtime acceptance is explicitly gated when unavailable", () => {
    assert.ok(process.platform !== "darwin"
      || process.env.CODEX_DREAM_SKIN_SKIP_SIGNED_RUNTIME_TESTS === "1");
  });
} else {
  test("media import wrapper requires the signed bundled Node runtime", async () => {
