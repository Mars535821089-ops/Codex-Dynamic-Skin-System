...
export async function runRendererRuntimeTest(assetRoot, expectedModules = [
  "module-registry.js",
  "media-layer.js",
  "audio-bus.js",
  ...
  const dynamicManifest = JSON.parse(await fs.readFile(
    path.join(assetRoot, "dynamic-runtime-manifest.json"), "utf8",
  ));
  fixture.template = template;

  assert.equal(dynamicManifest.schema, "codex-dynamic-skin-runtime/1");
  assert.deepEqual(dynamicManifest.modules.map((entry) => entry.name), expectedModules);
  for (const entry of dynamicManifest.modules) {
    const source = await fs.readFile(path.join(assetRoot, entry.path), "utf8");
  ...
  assert.match(css, /--ds-rendered-art:\s*var\(--dream-skin-art\)/);
  assert.match(css, /data-dynamic-skin-video-live="true"[\s\S]*--ds-rendered-art:\s*none/);
  assert.match(css, /background-image:\s*var\(--ds-task-full-veil\),\s*var\(--ds-rendered-art\)/);
  assert.match(
    css,
  ...
  assert.match(
    css,
    /\[data-codex-composer-root\]\[data-composer-placement="home"\]\s*>\s*\[data-composer-rail\]\[data-composer-rail-placement="above"\][\s\S]{0,220}width:\s*100%\s*!important;[\s\S]{0,120}margin-inline:\s*0\s*!important;/,
    "An inset native Home rail must occupy the same inline geometry as the lower composer surface.",
  );
  assert.match(
    css,
  ...
  const asyncReplacement = makeFixture({ nativeAppearance: "dark" });
  vm.runInNewContext(asyncReplacement.payloadFor({ id: "first" }), asyncReplacement.context);
  const asyncOldState = asyncReplacement.window.__CODEX_DREAM_SKIN_STATE__;
  const oldObservers = [...asyncReplacement.observers];
  const oldCleanup = asyncOldState.cleanup;
  let releaseOldCleanup;
  const oldCleanupGate = new Promise((resolve) => { releaseOldCleanup = resolve; });
  asyncOldState.cleanup = async () => {
    await oldCleanupGate;
    return oldCleanup();
  };
  vm.runInNewContext(asyncReplacement.payloadFor({ id: "second" }), asyncReplacement.context);
  assert.ok(oldObservers.every((observer) => observer.disconnected),
    "Replacing a theme must synchronously retire every observer from an async old generation");
  assert.equal(asyncReplacement.intervals.size, 1,
    "Replacing a theme must synchronously retire the old safety writer before installing the new one");
  assert.equal(asyncReplacement.rootStyle.values.get("--dream-skin-art"), 'url("blob:fixture-2")');
  releaseOldCleanup();
  await oldCleanupGate;
  await Promise.resolve();
  assert.equal(asyncReplacement.intervals.size, 1,
    "A late async cleanup must not leave an old safety writer or remove the replacement writer");
  assert.equal(asyncReplacement.rootStyle.values.get("--dream-skin-art"), 'url("blob:fixture-2")',
    "A late async cleanup must not remove the replacement artwork");
