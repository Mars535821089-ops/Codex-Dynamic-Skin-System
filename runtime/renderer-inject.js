...
((cssText, artDataUrl, themeConfig) => {
  ...
  if (typeof nativeState?.cleanup === "function") {
    try {
      const cleanupResult = nativeState.cleanup();
      if (cleanupResult && typeof cleanupResult.catch === "function") cleanupResult.catch(() => {});
    } catch {}
  }
  ...
  if (excludedPetSurface) {
    ...
    if (typeof previous?.retireLegacy === "function") previous.retireLegacy();
    if (typeof previous?.cleanup === "function") {
      try {
        const cleanupResult = previous.cleanup();
        if (cleanupResult && typeof cleanupResult.catch === "function") cleanupResult.catch(() => {});
      } catch {}
    }
  ...
  const retireLegacy = () => {
    ...
    if (navigationHandler && navigationApi) {
      try { navigationApi.removeEventListener("navigate", navigationHandler); } catch {}
    }
    if (styleSheet) {
      try {
    ...
    if (styleRegistry.size === 0 && window[STYLE_REGISTRY_KEY] === styleRegistry) {
      delete window[STYLE_REGISTRY_KEY];
    }
  ...
  const cleanup = () => {
    const state = window[STATE_KEY];
    const ownsState = state?.installToken === installToken;
    retireLegacy();
  ...
  window[STATE_KEY].timer = safetyTimer;
